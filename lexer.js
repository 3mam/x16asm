...
const mapFindLabelInValue = list => tokenData => {
	if (list.includes(tokenData.token))
...
const mapFindConstInValue = list => tokenData => {
	if (list.includes(tokenData.token) && tokenData.type !== types.CONST)
...
const mapFindCpuInstructionsInToken = cpuInstructions => tokenData => {
	if (typeof cpuInstructions[tokenData.token] === 'object')
...
const reduceCollectLabel = (list, tokenData) => {
	if (tokenData.type === types.LABEL)
		return [...list, tokenData.token.substring(0, tokenData.token.length - 1)]
	else
		return list
}

const reduceCollectConst = (list, tokenData) => {
	if (tokenData.type === types.CONST)
		return [...list, tokenData.token.substring(0, tokenData.token.length)]
	else
		return list
}
...
const filterRemoveNotImportantTokens = (tokenData) => {
	...
	return !typesToIgnore.includes(tokenData.type)
}
